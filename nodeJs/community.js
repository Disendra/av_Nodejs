...
router.post('/updateCommunity', upload.single('image'), (req, res) => {
 ...
 function updateCommunity() {
    const ques_postedDate = new Date().toISOString().slice(0, 19).replace('T', ' '); // Convert to MySQL compatible format
    const sql = `UPDATE community_questions SET emailId = '${emailId}', question = '${question}', ques_postedDate = '${ques_postedDate}', urlLink = '${urlLink}', qId = '${qId}', imagePath = '${imagePath}' WHERE qId = '${qId}'`;
